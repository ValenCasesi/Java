...
public class DataUsuario {
	...
	public LinkedList<Usuario> getAll(){
		...
		try {
			...
			rs= stmt.executeQuery("select dni,nombre,apellido,direccion,telefono,contrasenia,email,idRol from usuario");
			...
			if(rs!=null) {
				while(rs.next()) {
					...
					p.setRol(dr.getRoles(idRol)); // NO LISTA LOS USUARIOS POR ESTO PREGUNTAR A MECA
					usus.add(p);
				}
			}
	...
	public Usuario getByUser(Usuario usu) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(
					"select dni,nombre,apellido,direccion,telefono,contrasenia,email,idRol from usuario where email=? and contrasenia=?"
					);
	...
	public Usuario getByDni(Usuario usu) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(
					"select dni,nombre,apellido,direccion,telefono,contrasenia,email,idRol from usuario where dni=?"
					);
	...
	public void add(Usuario p) {
		...
		try {
			...
			// stmt.set(8, p.getRol()); Preguntar como setear el objeto rol.
			stmt.executeUpdate();
